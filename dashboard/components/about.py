...
def render_about_tab(region: str = "rangpur_rajshahi"):
    st.markdown(
        '<div style="background:#0d1822;border:1px solid #1e3a52;border-radius:8px;'
        'padding:16px 20px;margin-bottom:18px;">'
        '<div style="color:#f0f6ff;font-size:13px;line-height:1.7;">'
        ...
        '</div>'
        '<div style="color:#8ab4d4;font-size:12px;line-height:1.7;margin-top:10px;">'
    ...
    for i, (title, detail) in enumerate(METHOD_STEPS, start=1):
        st.markdown(
            ...
            f'<span style="color:#00d4ff;font-family:DM Mono,monospace;font-size:11px;'
            f'flex-shrink:0;width:22px;">{i:02d}</span>'
            f'<div><span style="color:#e8f4ff;font-size:12px;font-weight:600;">{title}</span>'
            f'<div style="color:#8ab4d4;font-size:11px;margin-top:2px;line-height:1.6;">'
    ...
    if val:
        ...
        st.markdown(
            f'<div style="color:#8ab4d4;font-size:12px;line-height:1.8;">'
            ...
            f'<b style="color:#e8f4ff;">AUC of {val.get("val_auc_roc", float("nan")):.3f}</b> '
    ...
    if hazard or source:
        ...
        st.markdown(
            ...
            f'<b style="color:#e8f4ff;">{readable}</b>.{extra} Kriging carries '
    ...
    if "auc_roc" in held:
        st.markdown(
            f'<div style="color:#8ab4d4;font-size:12px;line-height:1.8;margin-top:8px;">'
            ...
            f'<b style="color:#e8f4ff;">AUC of {held["auc_roc"]:.3f}</b>, and rank '
    ...
    for entry in region_status():
        color, text = (("#3cdea0", "available") if entry["available"]
                       else ("#dea03c", "in preparation"))
        st.markdown(
            f'<div style="display:flex;justify-content:space-between;'
            f'padding:7px 0;border-bottom:1px solid #1a2a38;font-size:12px;">'
            f'<span style="color:#e8f4ff;">{entry["name"]}'
            f'<span style="color:#5a8ab0;"> · {entry["hazard"]}</span></span>'
    ...
    for src in DATA_SOURCES_USED:
        st.markdown(
            f'<div style="border-bottom:1px solid #1a2a38;padding:8px 0;font-size:11px;">'
            f'<a href="{src["url"]}" target="_blank" rel="noopener noreferrer" '
            f'style="color:#e8f4ff;font-weight:600;text-decoration:none;">{src["name"]}</a>'
            f'<div style="color:#8ab4d4;margin-top:2px;">{src["use"]}</div>'
            f'<div style="color:#5a8ab0;font-size:10px;margin-top:2px;">'
    ...
    for limit in KNOWN_LIMITATIONS:
        st.markdown(
            f'<div style="color:#8ab4d4;font-size:11px;line-height:1.7;'
    ...
    st.markdown(
        f'<div style="color:#8ab4d4;font-size:11px;line-height:1.8;">'
        f'Source code: <a href="{REPO_URL}" target="_blank" rel="noopener noreferrer" '
        f'style="color:#00d4ff;">{REPO_URL}</a> (MIT licence)<br>'
        f'Input data archive: <a href="https://doi.org/{DATA_DOI}" target="_blank" '
        f'rel="noopener noreferrer" style="color:#00d4ff;">doi.org/{DATA_DOI}</a><br>'
