...
EDITS = [
    ...
    ("Every study cited here was checked against its DOI record, and the claims attributed to each are taken from its abstract.",
     "\\pending{Every study must be checked against its full text, not its abstract. Revise Table~\\ref{tab:litflood} and the sentences that rest on abstracts accordingly.}"),
