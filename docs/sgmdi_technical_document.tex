...
\newcommand{\headerlogo}{\includegraphics[height=10pt]{fermium_logo_header_black.png}}
...
Set against this literature, the system contributes three things that the reviewed Bangladesh studies do not report together in their abstracts. It trains and validates against flood extents observed by radar across several events, not a few hundred inventory points. It holds out whole areas when measuring accuracy, repeats that measurement over several block assignments, and reports calibrated probabilities alongside rankings together with a test of how far that calibration carries between areas. It also sets its own model against ordinary tabular baselines on identical features, a comparison none of the reviewed abstracts reports, and finds the graph architecture no better than a gradient-boosted tree. And it publishes its results openly with the data, code and a record of the errors found and corrected. It falls short in four places, the first being that its accuracy has not been compared with simpler models on the same data, so the choice of a graph network is not yet justified by results. Second, its hazard surface is no better than chance on the coast, where surge flooding depends on embankments and tides that the terrain predictors used here do not describe. Third, its landslide model uses one storm where a multi-year inventory exists, and fourth, its blocked validation, although stricter than a random split, is not a design-based estimate of map accuracy. Each is a concrete next step, and the first two are the ones a reviewer would raise first.
...
Every model in the chain is scored on spatial blocks held out from training, and the same block assignment is shared by the graph model, the terrain hazard model and the landslide model within a region. Metrics are the area under the ROC curve, which measures ranking, average precision, which is more sensitive to the rare positive class, and the Brier score for calibration. Average precision is also reported as a lift over the base rate, because an average precision of 0.07 against a positive rate of 0.01 is a sevenfold improvement over chance while the same value against a rate of 0.10 is worse than chance.

\subsection{Held-out results}

Table~\ref{tab:validation} reports the asset model and the terrain hazard model for each flood region, and the landslide model for the Hill Tracts. Scores are against the labels each region was trained on, which after the switch described in Section~\ref{sec:observed} are the observed extents. Where a region still reports against proxy labels the score carries the circularity described in Section~\ref{sec:labels}, and shows only that the model recovers the terrain rule. The two flood regions differ in how common flood-prone ground is under mapped assets. In Sylhet \SylhetLabelRate{}\,\% of assets sit on ground the ensemble calls flood-prone and the graph model reaches an AUC of \SylhetValAUC{}. In Rangpur and Rajshahi only \RRLabelRate{}\,\% do, the graph model still ranks well (AUC \RRValAUC{}) but its average precision of \RRValAP{} is modest in absolute terms because the positive class is so rare, and the terrain surface carries the spatial detail that the Kriged surface smooths away.
...
\toprule
Region & Assets & Positive rate & Graph AUC & Graph AP & Brier & Terrain AUC \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Validation on held-out 10\,km test blocks, for the single block assignment behind the published outputs. Positive rate is the share of assets whose ground the labels call flood-prone. The Brier score is that of the raw class-weighted scores, and the calibrated probabilities are reported in Section~\ref{sec:gnn}. Means over repeated assignments, which are lower, are in Table~\ref{tab:benchmark}.}
...
The limitations below are stated on the dashboard as well as here, and the first governs how every validation figure should be read. Flood labels come from terrain and historical surface water until the observed source has run. Because the same terrain enters the models, validation against those labels measures recovery of the labelling rule rather than skill at anticipating floods, and the observed-flood validation of Section~\ref{sec:observed} is the number that should be cited. Composite risk is defined only where OpenStreetMap has mapped assets, which in Rangpur and Rajshahi is 7\,\% of grid cells, and units without mapped assets are reported as missing rather than as low risk. Sparse mapping therefore reads as low exposure and not as low risk, and the infrastructure composite rises with asset density and is led by urban municipalities, which is the intended reading for infrastructure at risk and not a measure of risk to people, for which the population composite exists.

Vulnerability uses population density and three access distances, so income, housing quality and age structure are not represented, and WorldPop's constrained product misses settlement outside detected buildings. No model tried here beats a gradient-boosted tree on the same features, and the graph network the system began with is measurably worse in two of the three regions (Section~\ref{sec:benchmark}). The tree is therefore the default, and its advantage rests on the features rather than on anything the architecture contributes. Every metric from a single block assignment also carries a spread of about 0.02 to 0.04 in AUC, which is as large as several of the differences this document reports.

The Kriged hazard surface is informative only where flood-prone ground is spatially correlated at the spacing of mapped assets. Its variance-to-sill ratio is reported so the reader can judge, and the terrain surface is used where Kriging carries little. On the south-west coast the hazard surface is no better than chance against observed surge flooding (Section~\ref{sec:observed}) and the coastal composite should be read with that in mind. The raw asset scores are rankings. The calibrated probabilities of Section~\ref{sec:gnn} approximate likelihoods for the region as a whole and understate them where flood-prone ground is concentrated. The landslide surface describes one storm.
